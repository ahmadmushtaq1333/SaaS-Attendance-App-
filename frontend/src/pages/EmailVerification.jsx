...
export default function EmailVerification({ email, onVerificationSuccess, onCancel }) {
  ...
  const handleVerify = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.detail || err.response?.data?.error || "Invalid or expired activation code.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.detail || err.response?.data?.error || "Unable to resend OTP. Please try again.");
    } finally {
      setResending(false);
    }
  };
